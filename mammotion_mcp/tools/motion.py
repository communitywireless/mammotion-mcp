...
def register(
    ...
    *,
    ...
    Tier-3 (pause_mow, resume_mow) is always registered when this function
    is called — these are operational safety primitives that must be
    available even when the diagnostic flag is off.

    Tier-4 (manual_drive) is only registered when
    ``enable_diagnostic_tools=True``. The caller (server.py) reads the
    ``ENABLE_DIAGNOSTIC_TOOLS`` env var and passes the flag down.
    """

    # --- Tier 3 — pause / resume (always-on) ---
